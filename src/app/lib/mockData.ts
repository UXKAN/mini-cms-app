// Mock data — port van /tmp/anth-design/formulier/project/dashboard_data.js
// Gebruikt door alle pages voor preview/demo zolang Supabase data leeg is.
// Wanneer DEMO_MODE in useAuth.ts uit staat én er echte data in Supabase zit,
// kunnen pages deze mocks negeren.

export type DonorType = "particulier" | "ondernemer";
...
export type EventType = "religieus" | "fundraising" | "algemeen";
...
export type PromiseType = "cash" | "online" | "goud";
export type PromiseWanneer = "week" | "maand" | "jaar";
export type PromiseStatus = "open" | "voldaan";
...
function generateDonations(): Donation[] {
  ...
  donors.forEach((d) => {
    months.forEach(({ y, m }) => {
      ...
      result.push({
        id: `don${donId++}`,
        donorId: d.id,
        amount: d.bedrag_maand,
        date: `${y}-${String(m).padStart(2, "0")}-05`,
        type: "stripe",
        method: "online",
      });
    });
  });
  const cashExtras: Array<Omit<Donation, "id">> = [
  ...
  cashExtras.forEach((e) => result.push({ id: `don${donId++}`, ...e }));
  return result;
}

export const donations: Donation[] = generateDonations();
...
// Helpers
export function donationsForMonth(y: number, m: number): Donation[] {
...
export function totalForMonth(y: number, m: number): number {
...
export function businessCount(): number {
  return donors.filter((d) => d.actief && d.type === "ondernemer").length;
}

export function monthlyRecurring(): number {
